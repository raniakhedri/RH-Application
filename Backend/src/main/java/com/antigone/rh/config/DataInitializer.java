...
public class DataInitializer implements CommandLineRunner {
        ...
        private void initSystemParameters() {
                ...
                // ---------- Paramètres Agent Desktop & Monitoring ----------
                createParamIfNotExists("TOLERANCE_RETARD_MINUTES", "10",
                                "Tolérance en minutes avant de compter un retard",
                                TypeReferentiel.PARAMETRE_SYSTEME);

                createParamIfNotExists("POPUP_INTERVALLE_HEURES", "2",
                                "Intervalle en heures entre chaque popup de confirmation de présence",
                                TypeReferentiel.PARAMETRE_SYSTEME);

                createParamIfNotExists("POPUP_TIMEOUT_SECONDES", "60",
                                "Temps en secondes avant que le popup expire (confirmation ratée)",
                                TypeReferentiel.PARAMETRE_SYSTEME);

                createParamIfNotExists("INACTIVITE_TOLERANCE_MINUTES_JOUR", "30",
                                "Tolérance d'inactivité en minutes par jour avant signalement",
                                TypeReferentiel.PARAMETRE_SYSTEME);

                createParamIfNotExists("RESEAU_ENTREPRISE_IP", "192.168.1.0/24",
                                "Plage IP du réseau de l'entreprise pour vérification de connexion",
                                TypeReferentiel.PARAMETRE_SYSTEME);

                createParamIfNotExists("RESEAU_ENTREPRISE_SSID", "MonWiFi",
                                "Nom du réseau WiFi de l'entreprise pour vérification",
                                TypeReferentiel.PARAMETRE_SYSTEME);
        }
