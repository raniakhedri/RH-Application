        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-200 dark:border-gray-800">
          ...
          {showText && (
            <div className="overflow-hidden">
              ...
              <p className="text-theme-xs text-gray-500 dark:text-gray-400">Module RH</p>
            </div>
          )}
        </div>
            ...
            <div key={group.title} className="mb-4">
              ...
              <ul className="space-y-0.5">
                {group.items.map((item) => (
                  <SidebarItem
                    key={item.key}
                    item={item.key === 'validations' && canViewValidations && pendingCount > 0 ? { ...item, badge: pendingCount } : item}
                    isActive={isActive}
                    showText={showText}
